...
**Detector info**: This is where you can input the detector pixel size (sometimes called "pitch"), the distance from the aperture (or Fourier lens, if applicable) to the detector, and the wavelength of light used for illumination. The reconstructed pixel size $p_r$ can be calculated using the equation $p_r = \frac{d\lambda}{p_d N}$, where $d$ is the detector distance, $\lambda$ is the wavelength, $p_d$ is the detector's pixel size, and $N$ is the total number of pixels along one side of the image (i.e. for a 2x2 image, this would be 2, not 4).

**Subtract background**: Toggles whether the background is subtracted from the diffraction data. If no background image is loaded, this calculates the median amplitude of the image (usually a good indicator of the background) and subtract that constant value from all pixels. In either case, pixels that end up negative are set to zero. Background subtraction is probably the most important pre-processing feature in CDI, and should always be used when possible.

**Bin pixels**: Downsamples the image by combining adjacent pixels. Binning the diffraction pattern is roughly equivalent to cropping the reconstruction, except that the cropped edges get wrapped around to the other side of the image. This is useful for reducing data size (and, by extension, computational cost per iteration), but can prevent convergence if taken too far.

**Crop pixels**: Crops the diffraction pattern to an adjustable fraction of its original size. Cropping the diffraction pattern is roughly equivalent to downsampling the reconstruction. Like binning, this reduces the computational cost per iteration, but produces a lower-resolution image.
